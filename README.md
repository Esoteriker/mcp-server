...
> **当前状态：未在本机 Claude Code 注册。** 因为它复用的是 `gh` CLI 完整权限的登录态（`repo` scope 覆盖所有私有仓库读写，而不只是这 5 个只读工具用到的部分），风险和工具设计的只读范围不对等，所以先不接入。代码保留，之后如果换成范围更小的 fine-grained PAT（见下方命令的 `-e GH_TOKEN=...`）可以再注册。

```bash
claude mcp add github -e GH_TOKEN=<fine-grained PAT，仅 Metadata/Issues/Pull requests 只读> -- node github/build/index.js
```
